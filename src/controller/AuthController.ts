...
export class AuthController {
    static signUp = async (req, res) => {
        const {perID, password, username, roles} = req.body;
    ...
    static signIn = async (req, res) => {
        const {perID, password} = req.body;

        const user = await getConnection().getRepository(User)
            .findOne({relations: ["roles"], where: {perID}});
        ...
        // token 생성
        const token = jwt.sign({ jti: user.id, perID: user.perID, roles: user.roles.map(role => role.name)},
            process.env.secret, {
